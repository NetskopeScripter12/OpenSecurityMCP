...
mcp = FastMCP("SecurityMCP", log_level="ERROR")
...
def edit_payload_contents(doc_id: str, old_str: str, new_str: str) -> str:
    ...
    if not old_str or old_str not in docs[doc_id]:
        raise ValueError("Replacement text must match a nonempty existing string")
    docs[doc_id] = docs[doc_id].replace(old_str, new_str)
    docs[doc_id] += f"\nModified (UTC): {datetime.now(timezone.utc).isoformat()}"
